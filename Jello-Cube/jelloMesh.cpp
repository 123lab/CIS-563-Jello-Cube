...
int JelloMesh::GetIndex(int i, int j, int k) const
{
    int cols = j;
    int rows = i*(m_cols+1);
    int stacks = k*(m_rows+1)*(m_cols+1);
                                 ...
                                 JelloMesh::Intersection& result)
{
    ...
    // TODO  
	vec3 partPos = vec3 (p.position[0], p.position[1], p.position[2]);
	//find out the center of cylinder 
	vec3 cylinderCenter = 0.5*(cylinderStart + cylinderEnd); 
	//calculate the range check 
...
{
	// TODO
	//move the initial verlet down for one step 
	float wcellsize = m_width / m_cols;
    float hcellsize = m_height / m_rows;
    float dcellsize = m_depth / m_stacks;
    
	Particle test1 = GetParticle(m_vparticles, 2,2,2);
	Particle test2 = GetParticle(m_vparticles, 4,4,4); 
	if((test1.position[0] == (-m_width*0.5f + wcellsize*2))
		&&(test1.position[1] == (1.5 + hcellsize*2))
		&&(test1.position[2] == (-m_depth*0.5f + dcellsize*2))
		&&(test2.position[0] == (-m_width*0.5f + wcellsize*4))
		&&(test2.position[1] == (1.5 + hcellsize*4))
		&&(test2.position[2] == (-m_depth*0.5f + dcellsize*4)))
	{
		for (int i = 0; i < m_rows+1; i++)
			{
			for (int j = 0; j < m_cols+1; j++)
			{
				for (int k = 0; k < m_stacks+1; k++)
				{
					Particle &change = GetParticle(m_vparticles, i,j,k);
					change.position = 0.5*dt*dt*change.force * 1/change.mass;
					change.velocity = dt*change.force * 1/change.mass;
				}
			}
		}  
	}

	ParticleGrid Grid = m_vparticles;  //copy 
	calculateGrid(Grid, dt);
	for (int i = 0; i < m_rows+1; i++)
    {
        for (int j = 0; j < m_cols+1; j++)
        {
            for (int k = 0; k < m_stacks+1; k++)
            {
                Particle& p1 = GetParticle(m_vparticles, i,j,k);
				Particle& p2 = GetParticle(Grid, i,j,k);
                
				p1.velocity = p1.velocity+p1.velocity-p2.velocity+ dt*p1.force * 1/p1.mass;            
				p1.position = p1.position+p1.position-p2.position+ dt*p1.velocity;   
...
void JelloMesh::calculateGrid(ParticleGrid& grid, double dt)
{
	ParticleGrid target = grid;  // target is a copy!    
    ParticleGrid& source = grid;  // source is a ptr!  

    // Step 1   
    ParticleGrid accum1 = grid;
    for (int i = 0; i < m_rows+1; i++)
    {
        for (int j = 0; j < m_cols+1; j++)
        {
...
void JelloMesh::calculatePosition(ParticleGrid& grid, double dt)
{
	ParticleGrid target = grid;  // target is a copy!    
    ParticleGrid& source = grid;  // source is a ptr! 
	...
	for (int i = 0; i < m_rows+1; i++)
    {
        for (int j = 0; j < m_cols+1; j++)
        {
            for (int k = 0; k < m_stacks+1; k++)
            {
				...
				vec3 LastStepP = p1.position; 
				p1.position = p1.position + dt*p1.velocity + 0.5*dt*dt* p1.force * 1/p1.mass;  
				p1.velocity = p1.velocity + 0.5*((p1.position-LastStepP)/2*dt*dt + p1.velocity/dt);
				p1.velocity = p2.velocity; 
				p1.position = p2.position; 
            }
        }
...
void JelloMesh::SymplecticEuler(double dt)
{
	ParticleGrid Grid = m_vparticles;  //copy 
	...
	for (int i = 0; i < m_rows+1; i++)
    {
        for (int j = 0; j < m_cols+1; j++)
        {
            for (int k = 0; k < m_stacks+1; k++)
            {
				...
				vec3 LastStepP = p1.position; 
				p1.position = p1.position + dt*p1.velocity;  
				p1.velocity = p1.velocity + 0.5*((p1.position-LastStepP)/2*dt*dt + p1.velocity/dt);
				p1.velocity = p2.velocity; 
				p1.position = p2.position; 
            }
...
void JelloMesh::VerletVolecityIntegrate(double dt)
{
	ParticleGrid Grid = m_vparticles;  //copy 
	calculateGrid(Grid, dt);
	for (int i = 0; i < m_rows+1; i++)
    {
        for (int j = 0; j < m_cols+1; j++)
        {
            for (int k = 0; k < m_stacks+1; k++)
            {
				...
				p1.position = p1.position + dt*p1.velocity + 0.5*dt*dt* p1.force * 1/p1.mass;  
				//p1.velocity = p2.velocity;
				p1.velocity = (p1.position -LastStepP)/dt;
				p1.velocity = p2.velocity; 
				p1.position = p2.position; 
            }
        }
