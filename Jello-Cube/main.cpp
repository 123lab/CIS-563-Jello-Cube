...
JelloMesh theJello;
Camera theCamera;
World theWorldGround("worlds/ground.xml");
//World theWorldCylinder("worlds/ThirdScene.xml");
//World theWorldCylinder("worlds/SecondScene.xml");
World theWorldCylinder("worlds/FirstScene.xml");
...
void onKeyboardCb(unsigned char key, int x, int y)
{
   ...
   else if (key == 'q') theJello.SetIntegrationType(JelloMesh::VERLET);
   else if (key == 'w') theJello.SetIntegrationType(JelloMesh::VERLETVELOCITY);
   else if (key == 't') theJello.SetIntegrationType(JelloMesh::LEAPFROG);
   else if (key == 'y') theJello.SetIntegrationType(JelloMesh::SYMPLETICEULER);
...
{
   glutTimerFunc(35, onTimerCb, 0);
   if (isRunning) 
   {
       theJello.Update(0.0050, theWorldGround); 
	   theJello.Update(0.0060, theWorldCylinder); 
...
void drawOverlay()
{
  ...
  glPushAttrib(GL_LIGHTING_BIT);
     ...
     case JelloMesh::VERLETVELOCITY: intstr = "VerletVelocity"; break;
     case JelloMesh::LEAPFROG: intstr = "Leapfrog"; break;
     case JelloMesh::SYMPLETICEULER: intstr = "SympleticEuler"; break;
     }
...
int main(int argc, char **argv)
{
	...
	glutAddMenuEntry("Verlet\t'q'", 'q');
	glutAddMenuEntry("VerletVelocity\t'w'", 'w');
	glutAddMenuEntry("LeapFrog\t't'", 't');
	glutAddMenuEntry("SymplecticEuler\t'y'", 'y');
