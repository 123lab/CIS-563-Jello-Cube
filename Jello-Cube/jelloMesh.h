...
public:
    ...
    enum IntegrationType { EULER, MIDPOINT, RK4, VERLET, VERLETVELOCITY, LEAPFROG, SYMPLETICEULER};
...
protected:
	...
	virtual bool BoxIntersection(Particle& p, World::Cube* cube, Intersection& intersection);
	...
	virtual void VerletVolecityIntegrate(double dt);
	virtual void Leapfrog(double dt);
	virtual void SymplecticEuler (double dt);
	virtual void calculateGrid(ParticleGrid& grid, double dt);
	virtual void calculatePosition(ParticleGrid& grid, double dt);
